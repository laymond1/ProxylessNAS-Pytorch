...
class Network(nn.Module):
  ...
  def __init__(self, C_list, steps_list, strides_list, num_classes, criterion):
    ...
    self._num_classes = num_classes     # 6 for uci-har
    self._criterion = criterion
    
    # stem layer
    self.stem = nn.Sequential(
      nn.Conv1d(3, self._C_list[0], 3, stride=self._strides_list[0], padding=1, bias=False),
      nn.BatchNorm1d(self._C_list[0])
      )
    ...
    self.classifier = nn.Linear(self._C_list[5], num_classes)
